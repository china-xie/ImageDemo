...
@interface XTImagePickerViewController ()
...
@implementation XTImagePickerViewController
...
- (id)initWithImage:(UIImage *)originalImage withInterceptType:(ImageInterceptType)InterceptType cropFrame:(CGRect)cropFrame limitScaleRatio:(NSInteger)limitRatio {
    ...
    if (self) {
        ...
        switch (InterceptType) {
            case theSquareIntercept:
                self.cropFrame =    CGRectMake(0,(cropFrame.size.height-cropFrame.size.width)/2, cropFrame.size.width, cropFrame.size.width);
               
                break;
            case theRectangleIntercept:
                 self.cropFrame = CGRectMake(0, (cropFrame.size.height-cropFrame.size.width/2)/2, cropFrame.size.width, cropFrame.size.width/2.13);
                break;
            case theIDCardIntercept:
                 self.cropFrame = CGRectMake(0, (cropFrame.size.height-cropFrame.size.width/2)/2, cropFrame.size.width, cropFrame.size.width/1.7);
                break;
...
//    if (self.interceptType == theSquareIntercept ) {
////        self.oldFrame = CGRectMake(oriX, oriY, oriWidth, oriHeight);
//         self.ratioView = [[UIView alloc] initWithFrame:CGRectMake(self.cropFrame.origin.x, self.cropFrame.origin.y, self.cropFrame.size.width, self.cropFrame.size.height)];
//    } else if (self.interceptType ==theRectangleIntercept) {
//         self.ratioView =  [[UIView alloc] initWithFrame:CGRectMake(self.cropFrame.origin.x, self.cropFrame.origin.y + self.cropFrame.size.height/4, self.cropFrame.size.width, self.cropFrame.size.height/2)];
////        self.oldFrame = CGRectMake(oriX, oriY, oriWidth, oriHeight/2);
//    }
...
- (void)cancel:(id)sender {
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(XTImagePickerViewControllerDelegate)]) {
...
-(UIImage *)getSubImage{
    ...
    switch (_interceptType) {
        case theSquareIntercept:
            ...
            break;
        case theRectangleIntercept:
             myImageRect = CGRectMake(x, y, w, h/2.13);

            break;
        case theIDCardIntercept:
             myImageRect = CGRectMake(x, y, w, h/1.7);
